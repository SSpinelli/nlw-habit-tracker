  ...
  return (
    <main className="w-full flex">
      <div className="grid grid-rows-7 grid-flow-row gap-3">
        {
          daysArray.map((d, i) => (
            <div key={`${d}-${i}`} className="text-zinc-400 text-xl h-10 w-10 font-bold flex items-center justify-center">
              {d}
            </div>
          ))
        }
      ...
      <div className="grid grid-rows-7 grid-flow-col gap-3">
        ...
        {amountOfDaysToFill > 0 && Array.from({ length: amountOfDaysToFill }).map((_d, i) => {
          return (
            <div
              key={i}
